...
public class GareSelfRegistrationService {
    ...
    public GarePreviewResponse preview(String rawCode) {
        ...
        if (!p.isEnabled()) {
            throw new MobiliException(MobiliErrorCode.ACCESS_DENIED, "Cette compagnie n'accepte pas d'inscriptions");
        }
        p.getName();
    ...
    public AuthResponse register(GareSelfRegisterRequest req) {
        ...
        if (!partner.isEnabled()) {
            throw new MobiliException(MobiliErrorCode.ACCESS_DENIED, "Inscription indisponible pour cette compagnie");
        }
