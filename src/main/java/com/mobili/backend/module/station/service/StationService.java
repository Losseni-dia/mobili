...
public class StationService {
    ...
    public List<StationResponseDTO> listForCurrentUser(UserPrincipal principal) {
        ...
        if (u.getStation() != null) {
            ...
            return List.of(toDto(s));
        }
        return stationRepository.findByPartnerIdOrderByCityAscNameAsc(partner.getId()).stream()
                .map(this::toDto)
                .toList();
    }
    ...
    private void requirePartnerOwner(UserPrincipal principal) {
        User u = principal.getUser();
        if (u.getStation() != null) {
            throw new MobiliException(MobiliErrorCode.ACCESS_DENIED, "Réservé au dirigeant de la compagnie");
        }
